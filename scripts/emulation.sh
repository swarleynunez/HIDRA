...
nohup ../bin/geth --datadir "$(pwd)/N1" --networkid 12345 --bootnodes $BOOTNODE --maxpeers $PEERS \
...
do
  ...
  nohup ../bin/geth --datadir $PEER_DIR --networkid 12345 --bootnodes $BOOTNODE --maxpeers $PEERS \
...
# Compile HIDRA
env GOOS=darwin GOARCH=amd64 CGO_ENABLED=1 go build -o ../bin/hidra ../main.go
...
do
  ...
  sed -i '' -e '/ETH_NODE_DIR/d' .env  # MacOS X format
  sed -i '' -e '4s|^|ETH_NODE_DIR=\"'$PEER_DIR'\"\'$'\n|g' .env  # MacOS X format
